...
// 00:22 your device has a default databasae where it store everything from:
    // 1. system-wide defaults
    // 2. language defaults
    // 3. app-specific defaults (will be focuting on app0specific defautls)
    // 4. more ...

// 00:33 To access the values from this database, we'll need to call .standart from its UserDefaults object i.e. (see UserDefaults.standart...), which will return the "shared" defaults object that we need. Notice I said "shared" object, .standart will return a reference to the same user defaults object, regardless of where we invoke it in our program. This is called the "SINGLETON DESIGN PATTERN", check also in videos e.g.: https://www.youtube.com/watch?v=x6gbDWFNin4 or https://www.youtube.com/watch?v=hs0tGkPdSQM
// 00:50 Once we have the shared user defaults object, we can then use "keys to get values" (see value(forKey:...) for this default's object just like a dictionary. In this example, we'll use the method "value(forKey: Sting)", but there are other methods available as well such as string(forKey: String), bool(forKey: String), float(forKey: String), and others...
// continue at time / 01:07 In this examaple ... in else block ... 
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let weWantMidnight = UserDefaults.standard.value(forKey: "midnightThemeOn") { // note*****
            ...
            if weWantMidnight as! Bool {
                ...
                switchToMidnight()
            } // added because of commenting out below section, comment out in case want to caome back to previou version without code in AppDelegate
                /*
                print("we like midnight mode")
            } else {
                // 02:44 If its FALSE, we do not really need to do anything because the UI is set to loadWithDaytime theme in general/by default. But let's just add some print statements here for both true and false to see what' happening. Continue at time // 02:52 Okay, so now I just need to set up an action ...
            ...
            // 01:31 Setting a value for UserDefaults is very similar to checking for value and the syntax is: "set with a value and then the key String we will want to use to retrieve it (i.e. UserDefaults.standart.set(Any?, key: "keyString")). So in our case, this will be a Bool. So we'll say false, because no one has flipped the midnight theme on yet. And for key we'll put in that "midnightThemeOn" String.
            ...
            // 02:12 So let's just add a print that shows us that this is the apps very first launch, and we know that after this first launch we will now have a value as your default for that midnightThemeOn key, that is false.
            ...
            // 05:44 We no longer need to check if there's a value for "weWantMidnight", because we know that there is, because we set it in the AppDelegate which launches first (so comment out also ...print("we like midnitht mode")} else { print("we like daylight mode")}... as commented out via /* ... */
            // 05:52 And you can see how this would be preferred, because then you can make sure things are set up at appLaunch rather than at viewLoad. And let's run it one more time. But first let's reset the contents and setting for Simulator Device. And launch and switch to midnight, let's close our app. Relaunch and we see that our preference is saved. Awesome.
               */
        }
        
    }

    // 02:52 Okay, so now I just need to set up an action for when switch gets flipped. Let me do that now. So when this action gets executed, I check my switch otulet to see whether it's truned off or on "if themeSwitch.isOn"..., and then I call the appropriate UI configuration method either switchToMidnight() or switchToDaytime()
    @IBAction func switchFlipped(sender: Any) {
        if themeSwitch.isOn {
            switchToMidnight()
            // 03:06 It's here that I want to update my new preference in my UserDefaults. So I'll add in a set value here to true for "midnightThemeOn".
...
// 04:06 But how come it didn't print "This is the first that app does launch" when we first ran it?  Well, because I had run this app with UserDefaults already implemented when I showed you the demo.
...
// What we need to do is reset the content settings of our simulator. We'll end up  doing this a lot throughout the course, so we've included the directions for this step in the accompanying text as well. It's pretty simple though:
    ...
    // now we can launch the app as if it's the first time it's on our device, and now we get this output that says "This is the first that app does launch"
...
// note*****
// Note: If you're wondering why in line 31 we used value(forKey:) as opposed to what seems like the more apt bool(forKey:) which we use within AppDelegate in line 22, the reason has to do with the purpose of this variable. We're using the "weWantMidnight" variable to give us two important pieces of information:
    // 1) whether the variable is set at all, and if so,
